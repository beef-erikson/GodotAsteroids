...
# Sets all the rocks various properties
func start(pos, vel, _size):
	# Starting position, size, mass and scale
	position = pos
	size = _size
	mass = 1.5 * size
	$Sprite.scale = Vector2(1, 1) * scale_factor * size
	
	# Sets CircleShape and CollisionShape to the radius of the rock
	...
	# Scales explosion to the rock's size
...
# Animation is done, delete the rock
